...
class ContentNormalizer extends ContentEntityNormalizer {
  ...
  /**
   */
  public function __construct(EntityManagerInterface $entity_manager, ConfigFactoryInterface $config_factory, EntityTypeBundleInfo $entity_type_bundle, EntityFieldManager $entity_field_manager) {
  ...
  public function normalizeEntity($uuid, $fields, $format, array $context, $entity_config = NULL) {
    ...
    foreach ($fields as $key => $field) {
      ...
      // Handle entity reference fields for types nodes, taxonomy terms and views.
      if (is_array($field)) {
        if (isset($field['purest_type']) || isset($field[0]) && isset($field[0]['purest_type'])) {
          ...
          foreach ($entities as $entity_key => $entity_reference) {
            switch ($entity_reference['target_type']) {
              ...
              case 'view';
                // For entity reference fields that reference views, load the view,
                // find the first available rest_export display and return the
                // rendered view.
                $display_id = NULL;
