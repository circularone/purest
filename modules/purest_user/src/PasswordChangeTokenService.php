...
class PasswordChangeTokenService implements PasswordChangeTokenServiceInterface {
  ...
  */
  ...
  public function sendPasswordChangeTokenEmail(UserInterface $account, $langcode = NULL) {
    ...
    // If the custom site notification email has not been set, we use the site default.
    if (empty($site_mail)) {
  ...
  public function token(&$replacements, $data, $options) {
    if (isset($data['user'])) {
      ...
      $activation_url = $token_service->replace('[purest:reset_url]');
      $replacements['[purest:password_reset_url]'] = $this->
  ...
  public function changePassword(UserInterface $account, $token, $timestamp, $password) {
    // Time out, in seconds, until activation token expires. 24 hours = 86400 seconds.
    $timeout = 86400;
    ...
    // If the current timestamp is smaller than the expiry timestamp, and the account is set,
    // and the provided token is equal to the hash of the account + provided timestamp
