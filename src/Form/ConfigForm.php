...
class ConfigForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    // $service = \Drupal::service('purest_user.validation');
    // $account = user_load(1);
    // $service->sendVerificationEmail($account);
    ...
    $form['normalizers']['normalize'] = [
      ...
      '#description' => $this->t('Purest inludes a typed data normalizer. It simplifies the structures returned for nodes and taxonomy terms in rest responses.'),
  ...
  public function validatePath(array &$element, FormStateInterface $form_state, array &$complete_form) {
    if (!empty($element['#value'])) {
      ...
      foreach ($parts as $key => $part) {
        ...
        else if (preg_match('/[^a-z_\-0-9]/i', $part)) {
